    ...
    def api_request(self):
        try:
            ...
            payload = {
                ...
                "email": f"{fake.email()}",
                "password": f"{fake.password(length=12, special_chars=True, digits=True, upper_case=True, lower_case=True),}",
                "phone": f"{fake.phone_number()}",
                "userStatus": random.choice([0, 1])
            }
            start_time = time.time()
            response = self.client.get(url=API_URL, json=payload, headers=HEADERS, name="TC1S1")  
            end_time = time.time()
...
# locust -f find_pet_by_id_get.py --users 1 --web-port=8090 --spawn-rate 1 --run-time 15m
